...
FillEpsilonPlot::FillEpsilonPlot(const edm::ParameterSet& iConfig)
{
 ...
 if(MakeNtuple4optimization_){
  ...
  Tree_Optim->Branch( "MC_pi0", "std::vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<double> > >",        &_MC_pi0);
  Tree_Optim->Branch( "MC_eta", "std::vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<double> > >",        &_MC_eta);
...
FillEpsilonPlot::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
 ...
 //---- initialize MC information dumber
 _MC_pi0.clear();
 _MC_eta.clear();
 ...
 if( isMC_ && MC_Asssoc_ ){
  ...
  //Find MC pi0 and eta
  math::XYZTLorentzVector temp_resonance;
  //     std::cout << " finding pi0 and eta ..." << std::endl;
  //     std::cout << " GenPars->size() = " << GenPars->size() << std::endl;
  for (auto& GenPar : *GenPars){
   //      std::cout << " GenPar.pdgId() = " << GenPar.pdgId() << " status = " << GenPar.status() << std::endl;
   if (GenPar.pdgId()==111 && GenPar.status() == 2) { //---- pi0, status 2 -> decayed particle, not hard
    temp_resonance.SetXYZT( GenPar.pt(), GenPar.p4().Eta(), GenPar.p4().Phi(), GenPar.p4().E() );
    _MC_pi0.push_back(temp_resonance);
   }
   if (GenPar.pdgId()==221 && GenPar.status() == 2) { //---- eta, status 2 -> decayed particle, not hard
    temp_resonance.SetXYZT( GenPar.pt(), GenPar.p4().Eta(), GenPar.p4().Phi(), GenPar.p4().E() );
    _MC_eta.push_back(temp_resonance);
   }
  }
